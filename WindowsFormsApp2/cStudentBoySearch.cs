...
{
    class cStudentBoySearch
    {
        cStudent student;
        string connection = "Server=127.0.0.1;Database=student;Uid=root;Pwd=root";
        public List<cStudent> BoySearch() // 학생객체정보를 리스트에 넣어서 반환
        { 
            string sex = "남자"; // 남자학생일 경우
           
            List<cStudent> cStudents = new List<cStudent>(); // 리스트생성
                ...
                using (MySqlConnection conn = new MySqlConnection(connection))
                {
                    ...
                    while (dr.Read())
                    {
                    student = new cBoy(); // 남자성적 조회니 남자 객체생성
